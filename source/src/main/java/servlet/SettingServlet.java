...
public class SettingServlet extends HttpServlet {
	...
	//パスワードが入力されている場合に、現在のパスワードと一致しているか判定を行う。
	if (password1 != ""){
		//現在のパスワードがあっているかを確認して、合っていれば変更処理を行う。
			...
			if (isAuth) { // ログイン成功
				...
				RequestDispatcher dispatcher =   // 設定画面へ戻す
						request.getRequestDispatcher(view);
				dispatcher.forward(request, response);
			}
			
			//パスワードが入力されていない場合に、そのまま更新処理を行う。
		}else {
			if (uDAO.passupdate(new UserDTO(user_id,password2,nickname,prefecture_id))) { // 更新成功
				System.out.println("更新処理に成功しました。");
				UserDTO loginUser = uDAO.getLoginUser(user_id); 
				// セッションスコープにIDを格納する
				HttpSession session = request.getSession();
